...
# Suppress harmless divide-by-zero or invalid sqrt warnings
...
def TRVDRV(velocity_model, Z, DELTA, ELEV, VIDXS, ISW='', verbose=False):
	...
	eps = 1e-8  # small floor to prevent divide-by-zero
	for KV in range(NV):
		for LR in range(1, NL):
			...
			for LS in range(NL):
				...
				for LL in range(LS, LR):
					...
					dv2 = VSQ[KV, LR] - VSQ[KV, LL]
					# Skip physically invalid or flat layers
					if dv2 <= 0:
						continue
					SQT = np.sqrt(dv2 + eps) # small floor to prevent
					TIM = THK[LL] / V[KV, LL] * V[KV, LR] / SQT
					DIM = THK[LL] * V[KV, LL] / SQT
					TXD[KV, LS, LR] += TIM
					DXD[KV, LS, LR] += DIM
...
def PREP1(Z, D, V, VSQ, TID, DID):
	...
	if JL != NL - 1:
		for L in range(JJ, NL):
			dv2 = VSQ[L] - VSQ[JL]
			if dv2 <= 0:
				SQT = 1e-8   # tiny stabilizer, keeps Fortran logic intact
			else:
				SQT = np.sqrt(dv2)
...
def DIRWAV(Z, JL, TKJ, ZSTAT, JS, TKJS, TINS, DIDS, DELTA, TMIN, FF,
	...
	if JL == JS:
		...
		for L in range(JS, J1):
			term_big = VSQ[JL] / VSQ[L] - UBSQ
			term_lit = VSQ[JL] / VSQ[L] - ULSQ
			if term_big <= 0:
				term_big = 1e-8
			if term_lit <= 0:
				term_lit = 1e-8

			DELBIG += (THK[L] * UB) / np.sqrt(term_big)
			DELLIT += (THK[L] * UL) / np.sqrt(term_lit)
